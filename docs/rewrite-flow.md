...
`-merge-idle-timeout` is an inactivity window used to check whether the destination has reached the merge target. It is extended when ClickHouse has active destination merges or when the destination part snapshot changes. `-merge-max-runtime` is the hard cap for the whole source rewrite wait. When worker compaction is enabled, the initial source rewrite merge wait is capped at 5 minutes because later compaction work is responsible for deeper consolidation. The compaction path uses the remaining `-compact-window` as its hard merge-wait cap; the deprecated `-compact-merge-idle-timeout` and `-compact-merge-max-runtime` flags are ignored.
